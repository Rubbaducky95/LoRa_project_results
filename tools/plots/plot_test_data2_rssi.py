...
OUTPUT_PNG = r"C:\Users\ruben\Documents\LoRa Project\results\plots\test_data2_rssi_tp12_plots.png"
...
def collect_data():
    # data[bw][sf][tp] = list[(distance, mean_rssi)]
        ...
        for walk_root, _, files in os.walk(folder):
            for fn in files:
                ...
                if tp not in TP_VALUES or bw not in BWS_TO_PLOT:
                    continue
                mean_rssi = mean_rssi_from_file(os.path.join(walk_root, fn))
                if mean_rssi is None:
                    continue
                data[bw][sf][tp].append((distance, mean_rssi))
...
def plot():
    ...
    for row_idx, (ax, bw) in enumerate(zip(axes, BWS_TO_PLOT)):
        ...
        all_distances = sorted({
            d for sf in sf_series for tp in sf_series[sf] for d, _ in sf_series[sf][tp]
        })
    ...
    sf_handles = [
        Line2D([0], [0], color=color_map[sf], lw=2, label=f"SF{sf}")
        for sf in SF_VALUES
    ]
    tp_handles = [
        Line2D([0], [0], color="black", label=f"TP={tp}", **tp_style[tp])
        for tp in TP_VALUES
    ]

    fig.suptitle("Average RSSI vs Distance by SF", y=0.995)
