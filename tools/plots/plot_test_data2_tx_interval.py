...
OUTPUT_PNG = r"C:\Users\ruben\Documents\LoRa Project\results\plots\test_data2_tx_interval_plots.png"

SF_VALUES = [7, 8, 9, 10, 11, 12]
TP_VALUES = [2, 12, 22]
BWS_TO_PLOT = [125000, 250000, 500000]
...
def collect_data():
    # data[bw][sf][tp] = list[(distance, mean_time_ms)]
        ...
        for walk_root, _, files in os.walk(folder):
            for fn in files:
                ...
                if tp not in TP_VALUES or bw not in BWS_TO_PLOT:
                    continue
                mean_t = mean_time_from_file(os.path.join(walk_root, fn))
                if mean_t is None:
                    continue
                data[bw][sf][tp].append((distance, mean_t))
...
def plot():
    ...
    for ax, bw in zip(axes, BWS_TO_PLOT):
        ...
        ax.set_title(f"BW={bw} Hz (TP 2/12/22)")
    ...
    axes[0].set_ylabel("Average TX interval (ms)")
    ...
    fig.suptitle("Average TX interval vs Distance by SF/TP", y=0.995)
    fig.legend(sf_handles, [h.get_label() for h in sf_handles], loc="upper center", bbox_to_anchor=(0.33, 0.955), ncol=6, frameon=False)
    fig.legend(tp_handles, [h.get_label() for h in tp_handles], loc="upper center", bbox_to_anchor=(0.80, 0.955), ncol=3, frameon=False)
