...
OUTPUT_PNG = r"C:\Users\ruben\Documents\LoRa Project\results\raw_test_data_plots\raw_per_bw_vs_sf_scatter.png"
...
def collect_summary():
    # grouped[(bw, sf)] -> [per values across all distances + TP]
    ...
    for dn in sorted(os.listdir(DATA_ROOT)):
        ...
        for root, _, files in os.walk(dpath):
            for fn in files:
                ...
                if per is None:
                    continue
                grouped[(bw, sf)].append(per)

    # summary[bw] = [(sf, avg_per), ...]
    summary = defaultdict(list)
    for (bw, sf), vals in grouped.items():
        if vals:
            summary[bw].append((sf, sum(vals) / len(vals)))
    for bw in summary:
...
def main():
    ...
    for bw in BW_VALUES:
        ...
        plt.scatter(xs, ys, s=60, color=colors[bw], alpha=0.9, label=f"BW {bw}")
