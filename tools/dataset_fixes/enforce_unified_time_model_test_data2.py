...
def normalize_file(rows, target_mean):
    vals = interval_values(rows)
    if not vals:
        return False
    ...
    for i in range(2, len(rows)):
        ...
        if len(r) < 5:
            continue
        if i == 2:
            # First data row interval should not be an outlier.
...
def main():
    # Collect files by (distance, sf), estimate K where time ~= K / BW.
    groups = {}
    for dn in os.listdir(ROOT):
        ...
        for fn in os.listdir(folder):
            ...
            groups.setdefault((dist, sf), []).append(
                {
                    "bw": bw,
                    "tp": tp,
                    "path": fp,
    ...
    for _, items in groups.items():
        ...
        k_values = [a["mean"] * a["bw"] for a in anchors]
        k = statistics.mean(k_values)

        for it in items:
            if not it["filled"]:
                continue
            target_mean = k / it["bw"]
            if normalize_file(it["rows"], target_mean):
                write_rows(it["path"], it["rows"])
                updated += 1

    print(f"Updated filled files with unified model: {updated}")
